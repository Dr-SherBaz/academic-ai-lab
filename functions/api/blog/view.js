...
export const onRequestGet = async ({ request, env }) => {
  ...
  const db = env.DB;
  if (!db) return json({ reads: 0 }, 200, { ...corsHeaders(origin), 'Cache-Control': 'no-cache, no-store, must-revalidate' });

  const row = await db.prepare(`SELECT reads FROM blog_metrics WHERE slug = ?`).bind(slug).first();
  return json({ reads: row?.reads || 0 }, 200, { ...corsHeaders(origin), 'Cache-Control': 'no-cache, no-store, must-revalidate' });
};
